...
const announcementsData = [
  {
    key: "item1",
    size: "110 KB",
    fileUrl: "#", // ใส่ลิงก์จำลองไว้สำหรับการทดสอบ
  },
  {
    key: "item2",
    size: "94 KB",
...
// คอมโพเนนต์แสดงผลประกาศและดาวน์โหลดไฟล์เอกสาร PDF
export function StudentAnnouncements() {
  const t = useT()
  const [previewPdf, setPreviewPdf] = useState<{ title: string; url: string } | null>(null)

  // ฟังก์ชันช่วยจัดการเปิดอ่านเอกสาร PDF
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
          <h2 className="text-balance text-2xl font-bold tracking-tight text-primary sm:text-3xl">
            {t("studentImprovement.announcements.title")}
          </h2>
          <p className="text-sm text-muted-foreground/80">
            {t("studentImprovement.announcements.desc")}
          </p>
        </div>
            ...
            {announcementsData.map((item) => (
              ...
              >
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="space-y-1">
                    <h3 className="font-bold text-primary text-sm sm:text-base leading-snug group-hover:text-accent transition-colors">
                      {t(`studentImprovement.announcements.items.${item.key}.title`)}
                    </h3>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Calendar className="h-3.5 w-3.5" />
                        {t(`studentImprovement.announcements.items.${item.key}.date`)}
                      </span>
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                  ...
                  <button
                    onClick={() => handlePreview(t(`studentImprovement.announcements.items.${item.key}.title`), item.fileUrl)}
                  ...
                  >
                    ...
                    {t("studentImprovement.announcements.read")}
                  </button>
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      if (item.fileUrl === "#") {
                        e.preventDefault()
                        handlePreview(t(`studentImprovement.announcements.items.${item.key}.title`), item.fileUrl)
                      }
                    }}
                    target="_blank"
                  ...
                  >
                    ...
                    {t("studentImprovement.announcements.download")}
                  </a>
                </div>
              </div>
            ))}
          ...
          >
            ...
            >
                ...
                {previewPdf.url === "#" ? (
                  ...
                  <div className="text-center p-8 max-w-md space-y-4">
                    ...
                    <h4 className="font-bold text-primary text-base">
                      {t("studentImprovement.announcements.noPdfTitle")}
                    </h4>
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      {t("studentImprovement.announcements.noPdfDesc")}
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="flex md:hidden flex-col justify-center items-center text-center p-6 max-w-sm space-y-6">
                      ...
                      <div className="space-y-2">
                        <h4 className="font-bold text-primary text-sm sm:text-base">
                          {t("studentImprovement.announcements.pdfReadyTitle")}
                        </h4>
                        <p className="text-xs text-muted-foreground leading-relaxed px-4">
                          {t("studentImprovement.announcements.pdfReadyDesc")}
                        </p>
                      </div>
                      <a
                        href={previewPdf.url}
                      ...
                      >
                        <Eye className="h-3.5 w-3.5" />
                        {t("studentImprovement.announcements.pdfOpenBtn")}
                      </a>
                    </div>
                  </>
                )}
              ...
              <div className="p-3 bg-secondary/30 border-t border-border/40 text-[10px] text-center text-muted-foreground">
                {t("studentImprovement.announcements.footerHelp")}
              </div>
            </div>
          </div>
        )}
