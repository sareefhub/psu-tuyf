...
const eligibilityKeys = ["item1", "item2", "item3", "item4", "item5"] as const

// คอมโพเนนต์แสดงผลคุณสมบัติผู้สมัครเข้าร่วมค่าย/กิจกรรมของโครงการพัฒนาศักยภาพนักเรียน
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
          <h2 className="text-balance text-2xl font-bold tracking-tight text-primary sm:text-3xl">
            {t("studentImprovement.eligibility.title")}
          </h2>
          <p className="text-sm text-muted-foreground/80">
            {t("studentImprovement.eligibility.desc")}
          </p>
        </div>

        {/* เลย์เอาต์แสดงผลคุณสมบัติ 5 ข้อ */}
          ...
          {eligibilityKeys.map((item, index) => (
            ...
            >
              ...
              <p className="text-xs text-foreground/90 leading-relaxed font-semibold">
                {t(`studentImprovement.eligibility.items.${item}`)}
              </p>
            </div>
          ))}
        </div>
