...
// คอมโพเนนต์แสดงผลประกาศและดาวน์โหลดไฟล์เอกสาร PDF สำหรับครู
            ...
            {announcementsData.map((item) => (
              ...
              >
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="space-y-1">
                    <h3 className="font-bold text-primary text-sm sm:text-base leading-snug group-hover:text-accent transition-colors">
                      {t(`teacherImprovement.announcements.items.${item.key}.title`)}
                    </h3>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Calendar className="h-3.5 w-3.5" />
                        {t(`teacherImprovement.announcements.items.${item.key}.date`)}
                      </span>
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                  ...
                  <button
                    onClick={() => handlePreview(t(`teacherImprovement.announcements.items.${item.key}.title`), item.fileUrl)}
                  ...
                  >
                    ...
                    {t("teacherImprovement.announcements.read")}
                  </button>
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      if (item.fileUrl === "#") {
                        e.preventDefault()
                        handlePreview(t(`teacherImprovement.announcements.items.${item.key}.title`), item.fileUrl)
                      }
                    }}
                    target="_blank"
                  ...
                  >
                    ...
                    {t("teacherImprovement.announcements.download")}
                  </a>
                </div>
              </div>
            ))}
                ...
                {previewPdf.url === "#" ? (
                  ...
                  <>
                    ...
                    <div className="flex md:hidden flex-col justify-center items-center text-center p-6 max-w-sm space-y-6">
                      ...
                      >
                        <Eye className="h-3.5 w-3.5" />
                        {t("teacherImprovement.announcements.pdfOpenBtn")}
                      </a>
                    </div>
                  </>
                )}
