      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {obligationsData.map((ob, index) => (
            <div key={index} className="bg-card border border-border/50 rounded-2xl p-6 shadow-sm space-y-4 hover:border-accent/30 transition-all">
              <div className="flex gap-3 items-start">
              ...
              {ob.subItemsTh && (
                <ul className="pl-9 list-disc space-y-1.5 border-t border-border/40 pt-3">
                  {t(ob.subItemsTh, ob.subItemsEn).map((sub, sIdx) => (
                    <li key={sIdx} className="text-[11px] text-muted-foreground leading-relaxed font-semibold">
                      {sub}
                    </li>
                  ))}
                </ul>
